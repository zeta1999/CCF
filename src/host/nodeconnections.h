      ...
      DISPATCHER_SET_MESSAGE_HANDLER(
        disp, ccf::node_outbound, [this](const uint8_t* data, size_t size) {
          ...
          if (
            ...
            serialized::peek<consensus::ConsensusMsgType>(data, size) ==
              consensus::append_entries)
          {
            ...
            uint32_t frame = (uint32_t)(
              size_to_send + ledger.framed_entries_size(ae.prev_idx + 1, ae.idx));
            ...
            auto framed_entries = ledger.read_framed_entries(ae.prev_idx + 1, ae.idx);
